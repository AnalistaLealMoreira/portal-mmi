...
from accounts.models import RedePermitida
...
class EscopoDeAcessoTests(TestCase):
    ...
    def setUp(self):
        RedePermitida.objects.create(rede="127.0.0.1/32", descricao="Rede de testes")
        ...
        self.f_admin_empresa_a = Funcionario.objects.create(
            usuario=self.u_admin_empresa_a, empresa=self.empresa_a, setor=self.setor_a1
        )
    ...
    def test_diretor_ve_todos_os_links_da_propria_empresa(self):
        ...
        self.assertEqual(
            nomes,
            [
                "Dashboard Comercial A",
                "Dashboard Financeiro A",
                "Dashboard Financeiro A2",
            ],
        )

    def test_diretor_nao_acessa_link_de_outra_empresa(self):
        self.client.login(username="diretor_a", password="senha12345")
        resp = self.client.get(f"/links/{self.link_b1.pk}/acessar/")
        self.assertEqual(resp.status_code, 404)

    def test_admin_empresa_ve_todos_os_links_da_propria_empresa(self):
    ...
    def test_admin_empresa_nao_acessa_link_de_outra_empresa(self):
        self.client.login(username="admin_empresa_a", password="senha12345")
        resp = self.client.get(f"/links/{self.link_b1.pk}/acessar/")
        self.assertEqual(resp.status_code, 404)
